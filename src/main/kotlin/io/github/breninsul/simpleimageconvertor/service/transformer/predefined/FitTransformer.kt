...
/**
 * The `FitTransformer` class is a subclass of `PredefinedTransformer` that represents a transformer
 * for fitting an image into a specified resolution with additional settings.
 *
 * @param resolution The resolution to fit the image into. It is an instance of `FitSettings.Resolution`.
 * @param colour The color to use for the background if the image does not cover the entire resolution.
 * @param scaleMethod The scaling method to use for the image.
 * @param position The position to place the image within the resolution if the aspect ratios are different.
 *
